...
export class Kiosk extends APIResource {
   ...
   * locked app.
   */
  disable(deviceID: string, params: KioskDisableParams | null | undefined = {}, options?: RequestOptions): APIPromise<void> {
    const { 'X-Device-Display-ID': xDeviceDisplayID } = params ?? {}
    return this._client.delete(path`/devices/${deviceID}/kiosk`, { ...options, headers: buildHeaders([{Accept: '*/*', ...(xDeviceDisplayID?.toString() != null ? { 'X-Device-Display-ID': xDeviceDisplayID?.toString() } : undefined)}, options?.headers]) });
  }
  ...
  enable(deviceID: string, params: KioskEnableParams, options?: RequestOptions): APIPromise<void> {
    ...
    return this._client.put(path`/devices/${deviceID}/kiosk`, { body, ...options, headers: buildHeaders([{Accept: '*/*', ...(xDeviceDisplayID?.toString() != null ? { 'X-Device-Display-ID': xDeviceDisplayID?.toString() } : undefined)}, options?.headers]) });
  }
}
