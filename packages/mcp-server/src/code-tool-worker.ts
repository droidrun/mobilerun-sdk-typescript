// File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

import path from 'node:path';
import util from 'node:util';
import Fuse from 'fuse.js';
import ts from 'typescript';
import { WorkerOutput } from './code-tool-types';
import { Mobilerun, ClientOptions } from '@mobilerun/sdk';

async function tseval(code: string) {
  return import('data:application/typescript;charset=utf-8;base64,' + Buffer.from(code).toString('base64'));
}

function getRunFunctionSource(code: string): {
  type: 'declaration' | 'expression';
  client: string | undefined;
  code: string;
} | null {
  const sourceFile = ts.createSourceFile('code.ts', code, ts.ScriptTarget.Latest, true);
  const printer = ts.createPrinter();

  for (const statement of sourceFile.statements) {
    // Check for top-level function declarations
    if (ts.isFunctionDeclaration(statement)) {
      if (statement.name?.text === 'run') {
        return {
          type: 'declaration',
          client: statement.parameters[0]?.name.getText(),
          code: printer.printNode(ts.EmitHint.Unspecified, statement.body!, sourceFile),
        };
      }
    }

    // Check for variable declarations: const run = () => {} or const run = function() {}
    if (ts.isVariableStatement(statement)) {
      for (const declaration of statement.declarationList.declarations) {
        if (
          ts.isIdentifier(declaration.name) &&
          declaration.name.text === 'run' &&
          // Check if it's initialized with a function
          declaration.initializer &&
          (ts.isFunctionExpression(declaration.initializer) || ts.isArrowFunction(declaration.initializer))
        ) {
          return {
            type: 'expression',
            client: declaration.initializer.parameters[0]?.name.getText(),
            code: printer.printNode(ts.EmitHint.Unspecified, declaration.initializer, sourceFile),
          };
        }
      }
    }
  }

  return null;
}

function getTSDiagnostics(code: string): string[] {
  const functionSource = getRunFunctionSource(code)!;
  const codeWithImport = [
    'import { Mobilerun } from "@mobilerun/sdk";',
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Mobilerun)`
    : `const run: (${functionSource.client}: Mobilerun) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
  const sourcePath = path.resolve('code.ts');
  const ast = ts.createSourceFile(sourcePath, codeWithImport, ts.ScriptTarget.Latest, true);
  const options = ts.getDefaultCompilerOptions();
  options.target = ts.ScriptTarget.Latest;
  options.module = ts.ModuleKind.NodeNext;
  options.moduleResolution = ts.ModuleResolutionKind.NodeNext;
  const host = ts.createCompilerHost(options, true);
  const newHost: typeof host = {
    ...host,
    getSourceFile: (...args) => {
      if (path.resolve(args[0]) === sourcePath) {
        return ast;
      }
      return host.getSourceFile(...args);
    },
    readFile: (...args) => {
      if (path.resolve(args[0]) === sourcePath) {
        return codeWithImport;
      }
      return host.readFile(...args);
    },
    fileExists: (...args) => {
      if (path.resolve(args[0]) === sourcePath) {
        return true;
      }
      return host.fileExists(...args);
    },
  };
  const program = ts.createProgram({
    options,
    rootNames: [sourcePath],
    host: newHost,
  });
  const diagnostics = ts.getPreEmitDiagnostics(program, ast);
  return diagnostics.map((d) => {
    const message = ts.flattenDiagnosticMessageText(d.messageText, '\n');
    if (!d.file || !d.start) return `- ${message}`;
    const { line: lineNumber } = ts.getLineAndCharacterOfPosition(d.file, d.start);
    const line = codeWithImport.split('\n').at(lineNumber)?.trim();
    return line ? `- ${message}\n    ${line}` : `- ${message}`;
  });
}

const fuse = new Fuse(
  [
    'client.apps.confirmUpload',
    'client.apps.createSignedUploadURL',
    'client.apps.delete',
    'client.apps.list',
    'client.apps.listVersions',
    'client.apps.markFailed',
    'client.apps.retrieve',
    'client.apps.storageUsage',
    'client.carriers.create',
    'client.carriers.delete',
    'client.carriers.list',
    'client.carriers.lookup',
    'client.carriers.retrieve',
    'client.carriers.update',
    'client.credentials.list',
    'client.credentials.packages.create',
    'client.credentials.packages.list',
    'client.credentials.packages.listAll',
    'client.credentials.packages.credentials.create',
    'client.credentials.packages.credentials.delete',
    'client.credentials.packages.credentials.retrieve',
    'client.credentials.packages.credentials.fields.create',
    'client.credentials.packages.credentials.fields.delete',
    'client.credentials.packages.credentials.fields.update',
    'client.devices.count',
    'client.devices.create',
    'client.devices.fingerprint',
    'client.devices.list',
    'client.devices.reboot',
    'client.devices.reset',
    'client.devices.resume',
    'client.devices.retrieve',
    'client.devices.retrieveCapabilities',
    'client.devices.setName',
    'client.devices.stop',
    'client.devices.terminate',
    'client.devices.waitReady',
    'client.devices.actions.global',
    'client.devices.actions.overlayVisible',
    'client.devices.actions.setOverlayVisible',
    'client.devices.actions.swipe',
    'client.devices.actions.tap',
    'client.devices.apps.delete',
    'client.devices.apps.grantPermission',
    'client.devices.apps.install',
    'client.devices.apps.list',
    'client.devices.apps.listInstalls',
    'client.devices.apps.revokePermission',
    'client.devices.apps.start',
    'client.devices.apps.stop',
    'client.devices.esim.activate',
    'client.devices.esim.enable',
    'client.devices.esim.list',
    'client.devices.esim.remove',
    'client.devices.esim.setRoaming',
    'client.devices.esim.status',
    'client.devices.esim.apn.list',
    'client.devices.esim.apn.select',
    'client.devices.esim.apn.set',
    'client.devices.files.delete',
    'client.devices.files.download',
    'client.devices.files.list',
    'client.devices.files.upload',
    'client.devices.keyboard.clear',
    'client.devices.keyboard.key',
    'client.devices.keyboard.write',
    'client.devices.location.get',
    'client.devices.location.reset',
    'client.devices.location.set',
    'client.devices.packages.list',
    'client.devices.profile.update',
    'client.devices.proxy.connect',
    'client.devices.proxy.disconnect',
    'client.devices.proxy.status',
    'client.devices.state.screenshot',
    'client.devices.state.time',
    'client.devices.state.ui',
    'client.devices.tasks.list',
    'client.devices.clipboard.get',
    'client.devices.clipboard.set',
    'client.devices.timezone.get',
    'client.devices.timezone.set',
    'client.devices.language.get',
    'client.devices.language.set',
    'client.devices.deepLink.executeDeepLink',
    'client.devices.browser.executeScript',
    'client.devices.kiosk.disable',
    'client.devices.kiosk.enable',
    'client.devices.mediaSessions.activate',
    'client.devices.mediaSessions.create',
    'client.devices.mediaSessions.delete',
    'client.devices.mediaSessions.retrieveCurrent',
    'client.devices.recordings.delete',
    'client.devices.recordings.list',
    'client.devices.recordings.start',
    'client.devices.recordings.status',
    'client.devices.recordings.stop',
    'client.devices.recordings.trajectory',
    'client.devices.recordings.video',
    'client.models.list',
    'client.profiles.create',
    'client.profiles.delete',
    'client.profiles.list',
    'client.profiles.retrieve',
    'client.profiles.update',
    'client.proxies.create',
    'client.proxies.delete',
    'client.proxies.list',
    'client.proxies.lookup',
    'client.proxies.retrieve',
    'client.proxies.update',
    'client.connect.countries.list',
    'client.connect.proxies.buy',
    'client.connect.proxies.cancel',
    'client.connect.proxies.list',
    'client.connect.proxies.listConnections',
    'client.connect.proxies.ping',
    'client.connect.proxies.retrieve',
    'client.connect.users.create',
    'client.connect.users.delete',
    'client.connect.users.list',
    'client.connect.users.listConnections',
    'client.connect.users.retrieve',
    'client.connect.users.update',
    'client.tasks.attach',
    'client.tasks.getStatus',
    'client.tasks.getTrajectory',
    'client.tasks.list',
    'client.tasks.retrieve',
    'client.tasks.run',
    'client.tasks.runStreamed',
    'client.tasks.sendMessage',
    'client.tasks.stop',
    'client.tasks.screenshots.list',
    'client.tasks.screenshots.retrieve',
    'client.tasks.uiStates.list',
    'client.tasks.uiStates.retrieve',
    'client.workflows.triggers.create',
    'client.workflows.triggers.delete',
    'client.workflows.triggers.fire',
    'client.workflows.triggers.list',
    'client.workflows.triggers.retrieve',
    'client.workflows.triggers.update',
    'client.workflows.actionCatalog.list',
    'client.workflows.actionCatalog.retrieve',
    'client.workflows.actions.create',
    'client.workflows.actions.delete',
    'client.workflows.actions.list',
    'client.workflows.actions.retrieve',
    'client.workflows.actions.update',
    'client.workflows.actions.services.list',
    'client.workflows.actions.services.listMethods',
    'client.workflows.flows.clone',
    'client.workflows.flows.create',
    'client.workflows.flows.delete',
    'client.workflows.flows.dryRun',
    'client.workflows.flows.list',
    'client.workflows.flows.listRepairs',
    'client.workflows.flows.retrieve',
    'client.workflows.flows.unblock',
    'client.workflows.flows.update',
    'client.workflows.flows.actions.add',
    'client.workflows.flows.actions.list',
    'client.workflows.flows.actions.remove',
    'client.workflows.flows.actions.replace',
    'client.workflows.events.dryRun',
    'client.workflows.events.ingest',
    'client.workflows.executions.abort',
    'client.workflows.executions.getMetrics',
    'client.workflows.executions.list',
    'client.workflows.executions.retrieve',
    'client.workflows.timezones.list',
    'client.webhooks.create',
    'client.webhooks.delete',
    'client.webhooks.eventTypes',
    'client.webhooks.list',
    'client.webhooks.retrieve',
    'client.webhooks.rotateSecret',
    'client.webhooks.testDelivery',
    'client.webhooks.update',
    'client.webhooks.deliveries.list',
    'client.webhooks.deliveries.listForWebhook',
    'client.webhooks.deliveries.retrieveAttempts',
    'client.webhooks.deliveries.stats',
    'client.mailboxes.capacity',
    'client.mailboxes.create',
    'client.mailboxes.delete',
    'client.mailboxes.list',
    'client.mailboxes.otp',
    'client.mailboxes.restart',
    'client.mailboxes.retrieve',
    'client.mailboxes.uncancel',
    'client.mailboxes.update',
    'client.mailboxes.messages.list',
    'client.mailboxes.messages.retrieve',
    'client.files.cancelPending',
    'client.files.confirm',
    'client.files.delete',
    'client.files.download',
    'client.files.list',
    'client.files.update',
    'client.files.uploadURL',
    'client.assistant.conversations.abort',
    'client.assistant.conversations.answerPermission',
    'client.assistant.conversations.answerQuestion',
    'client.assistant.conversations.create',
    'client.assistant.conversations.history',
    'client.assistant.conversations.list',
    'client.assistant.conversations.rejectQuestion',
    'client.assistant.conversations.send',
    'client.assistant.conversations.stream',
    'client.assistant.conversations.update',
    'client.appEvents.list',
    'client.appEvents.retrieve',
    'client.appEvents.catalog.list',
    'client.appEvents.catalog.retrieve',
    'client.notifications.catalog',
    'client.notifications.getPreferences',
    'client.notifications.updatePreferences',
    'client.esims.capacity',
    'client.esims.confirmPayment',
    'client.esims.create',
    'client.esims.delete',
    'client.esims.import',
    'client.esims.install',
    'client.esims.installStatus',
    'client.esims.list',
    'client.esims.retrieve',
    'client.esims.selector',
    'client.esims.update',
    'client.esims.messages.list',
    'client.esims.messages.send',
    'client.messages.list',
    'client.messages.conversations.list',
    'client.messages.conversations.markRead',
    'client.numbers.countries',
    'client.numbers.create',
    'client.numbers.delete',
    'client.numbers.list',
    'client.numbers.purposes',
    'client.numbers.retrieve',
    'client.numbers.update',
    'client.numbers.messages.list',
    'client.store.categories',
    'client.store.apps.addToWorkspace',
    'client.store.apps.list',
    'client.store.apps.retrieve',
  ],
  { threshold: 1, shouldSort: true },
);

function getMethodSuggestions(fullyQualifiedMethodName: string): string[] {
  return fuse
    .search(fullyQualifiedMethodName)
    .map(({ item }) => item)
    .slice(0, 5);
}

const proxyToObj = new WeakMap<any, any>();
const objToProxy = new WeakMap<any, any>();

type ClientProxyConfig = {
  path: string[];
  isBelievedBad?: boolean;
};

function makeSdkProxy<T extends object>(obj: T, { path, isBelievedBad = false }: ClientProxyConfig): T {
  let proxy: T = objToProxy.get(obj);

  if (!proxy) {
    proxy = new Proxy(obj, {
      get(target, prop, receiver) {
        const propPath = [...path, String(prop)];
        const value = Reflect.get(target, prop, receiver);

        if (isBelievedBad || (!(prop in target) && value === undefined)) {
          // If we're accessing a path that doesn't exist, it will probably eventually error.
          // Let's proxy it and mark it bad so that we can control the error message.
          // We proxy an empty class so that an invocation or construction attempt is possible.
          return makeSdkProxy(class {}, { path: propPath, isBelievedBad: true });
        }

        if (value !== null && (typeof value === 'object' || typeof value === 'function')) {
          return makeSdkProxy(value, { path: propPath, isBelievedBad });
        }

        return value;
      },

      apply(target, thisArg, args) {
        if (isBelievedBad || typeof target !== 'function') {
          const fullyQualifiedMethodName = path.join('.');
          const suggestions = getMethodSuggestions(fullyQualifiedMethodName);
          throw new Error(
            `${fullyQualifiedMethodName} is not a function. Did you mean: ${suggestions.join(', ')}`,
          );
        }

        return Reflect.apply(target, proxyToObj.get(thisArg) ?? thisArg, args);
      },

      construct(target, args, newTarget) {
        if (isBelievedBad || typeof target !== 'function') {
          const fullyQualifiedMethodName = path.join('.');
          const suggestions = getMethodSuggestions(fullyQualifiedMethodName);
          throw new Error(
            `${fullyQualifiedMethodName} is not a constructor. Did you mean: ${suggestions.join(', ')}`,
          );
        }

        return Reflect.construct(target, args, newTarget);
      },
    });

    objToProxy.set(obj, proxy);
    proxyToObj.set(proxy, obj);
  }

  return proxy;
}

function parseError(code: string, error: unknown): string | undefined {
  if (!(error instanceof Error)) return;
  const cause = error.cause instanceof Error ? `: ${error.cause.message}` : '';
  const message = error.name ? `${error.name}: ${error.message}${cause}` : `${error.message}${cause}`;
  try {
    // Deno uses V8; the first "<anonymous>:LINE:COLUMN" is the top of stack.
    const lineNumber = error.stack?.match(/<anonymous>:([0-9]+):[0-9]+/)?.[1];
    // -1 for the zero-based indexing
    const line =
      lineNumber &&
      code
        .split('\n')
        .at(parseInt(lineNumber, 10) - 1)
        ?.trim();
    return line ? `${message}\n  at line ${lineNumber}\n    ${line}` : message;
  } catch {
    return message;
  }
}

const fetch = async (req: Request): Promise<Response> => {
  const { opts, code } = (await req.json()) as { opts: ClientOptions; code: string };

  const runFunctionSource = code ? getRunFunctionSource(code) : null;
  if (!runFunctionSource) {
    const message =
      code ?
        'The code is missing a top-level `run` function.'
      : 'The code argument is missing. Provide one containing a top-level `run` function.';
    return Response.json(
      {
        is_error: true,
        result: `${message} Write code within this template:\n\n\`\`\`\nasync function run(client) {\n  // Fill this out\n}\n\`\`\``,
        log_lines: [],
        err_lines: [],
      } satisfies WorkerOutput,
      { status: 400, statusText: 'Code execution error' },
    );
  }

  const diagnostics = getTSDiagnostics(code);
  if (diagnostics.length > 0) {
    return Response.json(
      {
        is_error: true,
        result: `The code contains TypeScript diagnostics:\n${diagnostics.join('\n')}`,
        log_lines: [],
        err_lines: [],
      } satisfies WorkerOutput,
      { status: 400, statusText: 'Code execution error' },
    );
  }

  const client = new Mobilerun({
    ...opts,
  });

  const log_lines: string[] = [];
  const err_lines: string[] = [];
  const originalConsole = globalThis.console;
  globalThis.console = {
    ...originalConsole,
    log: (...args: unknown[]) => {
      log_lines.push(util.format(...args));
    },
    error: (...args: unknown[]) => {
      err_lines.push(util.format(...args));
    },
  };
  try {
    let run_ = async (client: any) => {};
    run_ = (await tseval(`${code}\nexport default run;`)).default;
    const result = await run_(makeSdkProxy(client, { path: ['client'] }));
    return Response.json({
      is_error: false,
      result,
      log_lines,
      err_lines,
    } satisfies WorkerOutput);
  } catch (e) {
    return Response.json(
      {
        is_error: true,
        result: parseError(code, e),
        log_lines,
        err_lines,
      } satisfies WorkerOutput,
      { status: 400, statusText: 'Code execution error' },
    );
  } finally {
    globalThis.console = originalConsole;
  }
};

export default { fetch };
