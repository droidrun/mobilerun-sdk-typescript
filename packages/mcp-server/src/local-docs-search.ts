...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    params: ['id: string;'],
    response:
      "{ data: { id: string; appId: string; country: string; createdAt: string; createdBy: string; ownerId: string; queuedAt: string; sizeBytes: number; source: 'user' | 'system' | 'portal'; status: 'queued' | 'available' | 'failed'; targetSdk: number; updatedAt: string; userId: string; versionCode: number; versionName: string; }[]; }",
    markdown:
  ...
  },
  {
    ...
    params: [
      ...
      'country?: string;',
      "deviceType?: 'dedicated_premium_device' | 'dedicated_ios_device' | 'dedicated_emulated_device' | 'ios_simulator';",
      'profileId?: string;',
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; blockedAt: string; consecutiveFailures: number; cooldownScope: 'flow' | 'device'; cooldownSeconds: number; createdAt: string; createdBy: string; description: string; deviceIds: string[]; enabled: boolean; lastFailureAt: string; lastFailureCode: 'device_not_found' | 'permission_denied' | 'client_error' | 'transient' | 'logic' | 'invalid_config'; lastTriggeredAt: string; name: string; notifyOnFailure: boolean; notifyOnSuccess: boolean; notifyWebhookId: string; ownerId: string; status: 'healthy' | 'failing' | 'blocked'; templateResolutionVersion: number; triggerId: string; updatedAt: string; userId: string; }; }",
    markdown:
  ...
  },
  {
    ...
    response:
      "{ items: { id: string; blockedAt: string; consecutiveFailures: number; cooldownScope: 'flow' | 'device'; cooldownSeconds: number; createdAt: string; createdBy: string; description: string; deviceIds: string[]; enabled: boolean; lastFailureAt: string; lastFailureCode: 'device_not_found' | 'permission_denied' | 'client_error' | 'transient' | 'logic' | 'invalid_config'; lastTriggeredAt: string; name: string; notifyOnFailure: boolean; notifyOnSuccess: boolean; notifyWebhookId: string; ownerId: string; status: 'healthy' | 'failing' | 'blocked'; templateResolutionVersion: number; triggerId: string; updatedAt: string; userId: string; }[]; pagination: { hasNext: boolean; hasPrev: boolean; page: number; pages: number; pageSize: number; total: number; }; }",
    markdown:
