...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; cooldownScope: 'flow' | 'device'; cooldownSeconds: number; createdAt: string; description: string; enabled: boolean; lastTriggeredAt: string; name: string; triggerId: string; updatedAt: string; userId: string; }; }",
    markdown:
  ...
  },
  {
    ...
    response:
      "{ items: { id: string; cooldownScope: 'flow' | 'device'; cooldownSeconds: number; createdAt: string; description: string; enabled: boolean; lastTriggeredAt: string; name: string; triggerId: string; updatedAt: string; userId: string; }[]; pagination: { hasNext: boolean; hasPrev: boolean; page: number; pages: number; pageSize: number; total: number; }; }",
    markdown:
      "## list\n\n`client.workflows.flows.list(enabled?: boolean, orderBy?: 'name' | 'createdAt' | 'updatedAt', orderByDirection?: 'asc' | 'desc', page?: number, pageSize?: number, search?: string, triggerId?: string): { items: flow[]; pagination: pagination; }`\n\n**get** `/flows`\n\nList flows\n\n### Parameters\n\n- `enabled?: boolean`\n\n- `orderBy?: 'name' | 'createdAt' | 'updatedAt'`\n\n- `orderByDirection?: 'asc' | 'desc'`\n\n- `page?: number`\n\n- `pageSize?: number`\n\n- `search?: string`\n\n- `triggerId?: string`\n\n### Returns\n\n- `{ items: { id: string; cooldownScope: 'flow' | 'device'; cooldownSeconds: number; createdAt: string; description: string; enabled: boolean; lastTriggeredAt: string; name: string; triggerId: string; updatedAt: string; userId: string; }[]; pagination: { hasNext: boolean; hasPrev: boolean; page: number; pages: number; pageSize: number; total: number; }; }`\n\n  - `items: { id: string; cooldownScope: 'flow' | 'device'; cooldownSeconds: number; createdAt: string; description: string; enabled: boolean; lastTriggeredAt: string; name: string; triggerId: string; updatedAt: string; userId: string; }[]`\n  - `pagination: { hasNext: boolean; hasPrev: boolean; page: number; pages: number; pageSize: number; total: number; }`\n\n### Example\n\n```typescript\nimport Mobilerun from '@mobilerun/sdk';\n\nconst client = new Mobilerun();\n\nconst flows = await client.workflows.flows.list();\n\nconsole.log(flows);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    markdown:
      "## dry_run\n\n`client.workflows.events.dryRun(eventType: string, deviceId?: string, payload?: object): { data: object; }`\n\n**post** `/events/dry-run`\n\nSimulate an event against all configured flows. Returns which flows would match and what actions would run, without storing the event or enqueuing jobs.\n\n### Parameters\n\n- `eventType: string`\n\n- `deviceId?: string`\n\n- `payload?: object`\n\n### Returns\n\n- `{ data: { matchedFlows: { actions: object[]; flow: flow; trigger: object; }[]; validation: { valid: boolean; errors?: object[]; }; }; }`\n\n  - `data: { matchedFlows: { actions: { continueOnError: boolean; method: string; name: string; service: 'tasks_api' | 'devices_api' | 'agents_api' | 'webhooks'; deviceId?: string; params?: object; }[]; flow: { id: string; cooldownScope: 'flow' | 'device'; cooldownSeconds: number; createdAt: string; description: string; enabled: boolean; lastTriggeredAt: string; name: string; triggerId: string; updatedAt: string; userId: string; }; trigger: { id: string; activation: 'event' | 'schedule' | 'custom'; createdAt: string; customPayloadSchema: object; description: string; eventType: string; name: string; scheduleRule: { type: 'once' | 'cron' | 'recurring'; dateTime?: string; expression?: string; rrule?: string; }; timezone: string; updatedAt: string; userId: string; conditions?: object; nextFireTime?: string; }; }[]; validation: { valid: boolean; errors?: { field: string; message: string; }[]; }; }`\n\n### Example\n\n```typescript\nimport Mobilerun from '@mobilerun/sdk';\n\nconst client = new Mobilerun();\n\nconst response = await client.workflows.events.dryRun({ eventType: 'x' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
