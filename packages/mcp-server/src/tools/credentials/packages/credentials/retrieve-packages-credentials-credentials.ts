...
export const tool: Tool = {
  name: 'retrieve_packages_credentials_credentials',
  description:
    "When using this tool, always use the `jq_filter` parameter to reduce the response size and improve performance.\n\nOnly omit if you're sure you don't need the data.\n\nGet a specific credential with its fields\n\n# Response Schema\n```json\n{\n  $ref: '#/$defs/credential_retrieve_response',\n  $defs: {\n    credential_retrieve_response: {\n      type: 'object',\n      properties: {\n        data: {\n          $ref: '#/$defs/credential'\n        }\n      },\n      required: [        'data'\n      ]\n    },\n    credential: {\n      type: 'object',\n      properties: {\n        credentialName: {\n          type: 'string'\n        },\n        fields: {\n          type: 'array',\n          items: {\n            type: 'object',\n            properties: {\n              fieldType: {\n                type: 'string',\n                enum: [                  'email',\n                  'username',\n                  'password',\n                  'api_token',\n                  'phone_number',\n                  'two_factor_secret',\n                  'backup_codes'\n                ]\n              },\n              value: {\n                type: 'string'\n              }\n            },\n            required: [              'fieldType',\n              'value'\n            ]\n          }\n        },\n        packageName: {\n          type: 'string'\n        },\n        secretPath: {\n          type: 'string'\n        },\n        userId: {\n          type: 'string'\n        }\n      },\n      required: [        'credentialName',\n        'fields',\n        'packageName',\n        'secretPath',\n        'userId'\n      ]\n    }\n  }\n}\n```",
  inputSchema: {
    type: 'object',
    properties: {
      packageName: {
...
export const handler = async (client: DroidrunCloud, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(
        jq_filter,
        await client.credentials.packages.credentials.retrieve(credentialName, body),
      ),
    );
  } catch (error) {
    if (isJqError(error)) {
