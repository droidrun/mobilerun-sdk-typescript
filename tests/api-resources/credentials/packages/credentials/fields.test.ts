...
describe('resource fields', () => {
  ...
  test.skip('delete: only required params', async () => {
    const responsePromise = client.credentials.packages.credentials.fields.delete('email', { packageName: 'x', credentialName: '26f1kl_-n-71' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('delete: required and optional params', async () => {
    const response = await client.credentials.packages.credentials.fields.delete('email', { packageName: 'x', credentialName: '26f1kl_-n-71' });
  });
});
