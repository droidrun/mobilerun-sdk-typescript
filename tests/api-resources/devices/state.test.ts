...
describe('resource state', () => {
  ...
  test.skip('screenshot: request options and params are passed correctly', async () => {
    ...
    await expect(client.devices.state.screenshot('deviceId', { hideOverlay: true, 'X-Device-Display-ID': 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('time: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.devices.state.time('deviceId', { 'X-Device-Display-ID': 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Mobilerun.NotFoundError);
  });
  ...
  test.skip('ui: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.devices.state.ui('deviceId', { filter: true, 'X-Device-Display-ID': 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Mobilerun.NotFoundError);
  });
});
