...
describe('resource apn', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.devices.esim.apn.list('deviceId', { 'X-Device-Display-ID': 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('set: only required params', async () => {
    const responsePromise = client.devices.esim.apn.set('deviceId', {
    apn: 'apn',
    mcc: 'mcc',
    mnc: 'mnc',
    name: 'name',
  ...
  test.skip('set: required and optional params', async () => {
    const response = await client.devices.esim.apn.set('deviceId', {
    apn: 'apn',
    mcc: 'mcc',
    mnc: 'mnc',
    name: 'name',
