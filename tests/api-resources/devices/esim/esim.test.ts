...
describe('resource esim', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.devices.esim.list('deviceId', { 'X-Device-Display-ID': 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('setRoaming: required and optional params', async () => {
    const response = await client.devices.esim.setRoaming('deviceId', { enabled: true, 'X-Device-Display-ID': 0 });
  });
  ...
  test.skip('status: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.devices.esim.status('deviceId', { 'X-Device-Display-ID': 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Mobilerun.NotFoundError);
  });
});
